...
export const playerTurret: EnemyDefinition<PlayerTurretParams> = {
    ...
    update(state: GameState, enemy: Enemy<PlayerTurretParams>): void {
        const closestEnemy = getClosestElement(enemy, state.enemies.filter(e => !e.isInvulnerable));
        ...
        const attacksPerSecond = weapon.attacksPerSecond * state.hero.attacksPerSecond;
        ...
        for (const shot of weapon.shots) {
            ...
            if (shotTime >= attackTime - FRAME_LENGTH / 2 && shotTime < attackTime + FRAME_LENGTH / 2) {
                ...
                const bullet = shot.generateBullet(state, {...state.hero, x: enemy.x, y: enemy.y, theta: enemy.theta}, weapon);
