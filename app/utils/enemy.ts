...
import { getHeroLevelDamageBonus, getHeroWeaponProficiencyLevelDamageBonus, getBaseWeaponDpsForLevel } from 'app/utils/coreCalculations';
...
export function createEnemy<EnemyParams>(state: GameState, x: number, y: number, definition: EnemyDefinition<EnemyParams>, level: number, disc: Disc, stats?: Partial<Enemy>): Enemy<EnemyParams> {
    const heroBaseWeaponDamage = getBaseWeaponDpsForLevel(level);
    const heroLevelDamageFactor = getHeroLevelDamageBonus(level);
    const heroProficiencyDamageFactor = getHeroWeaponProficiencyLevelDamageBonus(level);
    ...
    const baseArmor = Math.ceil(heroDamage / 20 * (definition.statFactors.armor ?? 1));
    ...
    const enemy = {
        ...
        speed: window.BASE_ENEMY_SPEED * (definition.statFactors.speed ?? 1) * (0.9 + 0.2 * Math.random()),
        baseArmor,
        armor: baseArmor,
        damage: Math.floor(
            (heroMaxLife / 10 + heroMaxLife / 10 * level / 100 + 1.5 * level * halfHeavyArmorBonus) * (definition.statFactors.damage ?? 1)
        ),
        attacksPerSecond: (0.75 + 0.0125 * level) * (definition.statFactors.attacksPerSecond ?? 1),
        attackCooldown: 0,
